...
export default function POSPage() {
  ...
  const [lastSale, setLastSale] = useState<{ invoiceNumber: string; subtotal: number; taxAmount: number; total: number; paymentMethod: string; paymentStatus: string } | null>(null);
  ...
  const updateQuantity = (productId: number, delta: number) => {
    setCart((current) => current.flatMap((item) => {
      ...
      const step = item.unit === "kg" || item.unit === "litre" ? 0.01 : 1;
      const quantity = Math.round((item.quantity + delta * step) * 100) / 100;
  ...
  const setQuantity = (productId: number, value: string) => {
    ...
    setCart((current) => current.map((item) => {
      if (item.productId !== productId) return item;
      const step = item.unit === "kg" || item.unit === "litre" ? 0.01 : 1;
      const quantity = Math.round(parsed / step) * step;
      return { ...item, quantity: Math.max(step, Math.min(item.stock, Number(quantity.toFixed(2)))) };
    }));
  };
  ...
  const completeSale = async () => {
    ...
    try {
      ...
      setCart([]);
      setLastSale({ invoiceNumber: result.invoiceNumber, subtotal: Number(result.subtotal ?? subtotal), taxAmount: Number(result.taxAmount ?? taxAmount), total: Number(result.totalAmount ?? totalAmount), paymentMethod, paymentStatus: result.paymentStatus });
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="mt-2 text-slate-500">Hatua 1: bonyeza bidhaa ili iingie kwenye cart. Bei ni ya unit iliyowekwa kwenye catalog. Hatua 2: kagua quantity na jumla. Hatua 3: chagua njia ya malipo, kisha kamilisha mauzo.</p>
      </div>
