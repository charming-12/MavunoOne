...
type StaffUser = { id: number; name: string; email: string; phone: string | null; jobTitle: string | null; role: string; canPublishCatalog: boolean; isActive: boolean; createdAt: string };
...
export default function EmployeesPage() {
  ...
  const staffUsers = useMemo(() => users.filter((user) => !protectedRoles.has(user.role)), [users]);
  const removableUsers = useMemo(() => staffUsers.filter((user) => user.isActive && user.id !== actor?.id), [staffUsers, actor?.id]);
  const inactiveUsers = useMemo(() => users.filter((user) => !user.isActive && !protectedRoles.has(user.role) && user.id !== actor?.id), [users, actor?.id]);
  const selectedInactiveIds = selectedIds.filter((id) => inactiveUsers.some((user) => user.id === id));
  ...
  const assignCatalogPermission = async (user: StaffUser) => {
    setBusy(true); setMessage("");
    try {
      const response = await fetch("/api/admin/users", { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ userId: user.id, canPublishCatalog: !user.canPublishCatalog }) });
      const data = await response.json();
      if (!response.ok) throw new Error(data.message || "Responsibility could not be updated");
      setMessage(data.message); await loadUsers();
    } catch (error) { setMessage(error instanceof Error ? error.message : "Responsibility could not be updated"); }
  ...
  const permanentlyDeleteUsers = async (ids: number[]) => {
    if (!ids.length) return;
    if (!window.confirm(`Futa kabisa account ${ids.length === 1 ? "hii" : `hizi accounts ${ids.length}`}? Hatua hii haiwezi kurudishwa.`)) return;
    setBusy(true); setMessage("");
    try {
      const response = await fetch("/api/admin/users", { method: "DELETE", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ ids, permanent: true }) });
      const data = await response.json();
      if (!response.ok) throw new Error(data.message || "Accounts could not be permanently deleted");
      setMessage(data.message); await loadUsers();
    } catch (error) { setMessage(error instanceof Error ? error.message : "Accounts could not be permanently deleted"); }
    finally { setBusy(false); }
  };
  ...
  const createInvitation = async (event: FormEvent) => {
    ...
    if (form.phone.trim().length < 7) { setMessage("Namba ya simu ni lazima kwa account ya staff na itatumika kwa OTP ya password recovery."); return; }
  ...
  return <div className="space-y-6">
    ...
    {canProvision && <div className="rounded-2xl border border-sky-200 bg-sky-50 p-5 text-sm text-sky-900"><ShieldCheck className="mb-2 text-sky-700" size={20} /><p><strong>Jukumu maalum:</strong> Admin anaweza kubonyeza <strong>Mpe public shop</strong> kwa staff mmoja active. Huyo anakuwa Public Catalog Manager; Admin akimpa mwingine, ruhusa ya wa kwanza huondolewa moja kwa moja.</p></div>}
    ...
    {canProvision && <form onSubmit={createInvitation} className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm"><div className="mb-4 flex items-center gap-2"><UserPlus className="text-emerald-600" size={20} /><h2 className="text-lg font-black text-slate-900">Tengeneza staff account</h2></div><div className="grid gap-3 sm:grid-cols-2"><input required value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} placeholder="Jina kamili" className="rounded-lg border border-slate-200 px-3 py-3 text-sm outline-none focus:border-emerald-600" /><input required type="email" value={form.email} onChange={(e) => setForm({ ...form, email: e.target.value })} placeholder="Email ya staff" className="rounded-lg border border-slate-200 px-3 py-3 text-sm outline-none focus:border-emerald-600" /><input required type="tel" value={form.phone} onChange={(e) => setForm({ ...form, phone: e.target.value })} placeholder="Namba ya simu ya staff (lazima)" className="rounded-lg border border-slate-200 px-3 py-3 text-sm outline-none focus:border-emerald-600" /><select value={form.jobTitle} onChange={(e) => { const jobTitle = e.target.value; setForm({ ...form, jobTitle, role: jobTitle === "finance" || jobTitle === "operations_manager" ? "manager" : jobTitle }); }} className="rounded-lg border border-slate-200 px-3 py-3 text-sm outline-none focus:border-emerald-600"><option value="finance">Finance — Manager permissions</option><option value="operations_manager">Operations Manager</option><option value="storekeeper">Storekeeper</option><option value="cashier">Cashier</option><option value="machine_operator">Machine Operator</option></select></div><button type="submit" className="mt-4 inline-flex items-center gap-2 rounded-xl bg-emerald-700 px-4 py-3 font-bold text-white hover:bg-emerald-800"><UserPlus size={18} />Create invitation</button></form>}
    <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm"><div className="mb-4 flex flex-wrap items-center justify-between gap-3"><div><h2 className="text-lg font-black text-slate-900">Staff wa Office</h2><p className="mt-1 text-xs text-slate-500">Hapa unaona waajiriwa wa kazi za kila siku. System Admin, Owner na Boss ni system accounts zilizotenganishwa na roster hii.</p></div><div className="flex flex-wrap items-center gap-2"><span className="rounded-full bg-emerald-50 px-3 py-1 text-xs font-bold text-emerald-700">{staffUsers.filter((user) => user.isActive).length} active</span>{canProvision && selectedInactiveIds.length > 0 && <button type="button" disabled={busy} onClick={() => void permanentlyDeleteUsers(selectedInactiveIds)} className="inline-flex items-center gap-2 rounded-lg bg-red-800 px-3 py-2 text-xs font-bold text-white hover:bg-red-900 disabled:opacity-50"><Trash2 size={14} />Futa kabisa ({selectedInactiveIds.length})</button>}{canProvision && <button type="button" disabled={cleaning} onClick={() => void removeSeededMockAccounts()} className="rounded-lg border border-red-200 px-3 py-2 text-xs font-bold text-red-700 hover:bg-red-50 disabled:opacity-50">{cleaning ? "Inaondoa..." : "Ondoa sample accounts"}</button>}{canProvision && selectedIds.filter((id) => removableUsers.some((user) => user.id === id)).length > 0 && <button type="button" disabled={busy} onClick={() => void removeUsers(selectedIds.filter((id) => removableUsers.some((user) => user.id === id)))} className="inline-flex items-center gap-2 rounded-lg bg-red-700 px-3 py-2 text-xs font-bold text-white hover:bg-red-800 disabled:opacity-50"><Trash2 size={14} />Ondoa zilizochaguliwa ({selectedIds.length})</button>}</div></div>
      {loading ? <div className="flex items-center gap-2 py-10 text-slate-500"><Loader2 className="animate-spin" size={18} />Inapakia users...</div> : staffUsers.length === 0 ? <p className="py-10 text-center text-slate-500">Hakuna staff account iliyoundwa bado.</p> : <div className="overflow-x-auto"><table className="w-full min-w-[780px] text-left"><thead className="border-b border-slate-200 text-xs uppercase tracking-wide text-slate-500"><tr><th className="w-12 px-3 py-3"><button type="button" onClick={toggleAll} disabled={!canProvision || removableUsers.length === 0} aria-label="Select removable accounts" className="text-slate-500 disabled:opacity-30">{allRemovableSelected ? <Check size={18} /> : <Square size={18} />}</button></th><th className="px-3 py-3">Jina</th><th className="px-3 py-3">Email</th><th className="px-3 py-3">Simu</th><th className="px-3 py-3">Cheo / permissions</th><th className="px-3 py-3">Status</th><th className="px-3 py-3">Action</th></tr></thead><tbody className="divide-y divide-slate-100">{staffUsers.map((user) => { const protectedAccount = user.id === actor?.id; return <tr key={user.id} className={!user.isActive ? "bg-slate-50 opacity-60" : ""}><td className="px-3 py-4"><button type="button" onClick={() => toggleSelected(user.id)} disabled={!canProvision || protectedAccount} aria-label={`Select ${user.name}`} className="text-slate-500 disabled:opacity-30">{selectedIds.includes(user.id) ? <Check size={18} className="text-red-700" /> : <Square size={18} />}</button></td><td className="px-3 py-4 font-bold text-slate-800">{user.name}</td><td className="px-3 py-4 text-slate-600">{user.email}</td><td className="px-3 py-4 text-slate-600">{user.phone || "—"}</td><td className="px-3 py-4"><span className={`rounded-full px-2.5 py-1 text-xs font-bold ${protectedAccount ? "bg-blue-50 text-blue-800" : "bg-slate-100 text-slate-700"}`}>{titleLabels[user.jobTitle ?? ""] ?? roleLabels[user.role] ?? user.role}{user.canPublishCatalog && <span className="ml-1 text-[10px] opacity-70">· Public Catalog Manager</span>}{protectedAccount && <span className="ml-1 text-[10px] opacity-70">· protected</span>}</span></td><td className="px-3 py-4"><span className={`rounded-full px-2.5 py-1 text-xs font-bold ${user.isActive ? "bg-emerald-50 text-emerald-700" : "bg-slate-200 text-slate-600"}`}>{user.isActive ? "Active" : "Access removed"}</span></td><td className="px-3 py-4"><div className="flex flex-wrap gap-2">{canProvision && !protectedAccount && user.isActive && <><button type="button" disabled={busy} onClick={() => void assignCatalogPermission(user)} className={`rounded-lg border px-2.5 py-2 text-xs font-bold disabled:opacity-50 ${user.canPublishCatalog ? "border-amber-200 text-amber-800 hover:bg-amber-50" : "border-emerald-200 text-emerald-700 hover:bg-emerald-50"}`}>{user.canPublishCatalog ? "Ondoa public" : "Mpe public shop"}</button><button type="button" disabled={busy} onClick={() => void removeUsers([user.id])} className="inline-flex items-center gap-1.5 rounded-lg border border-red-200 px-2.5 py-2 text-xs font-bold text-red-700 hover:bg-red-50 disabled:opacity-50"><Trash2 size={14} />Ondoa</button></>}{canProvision && !protectedAccount && !user.isActive && <button type="button" disabled={busy} onClick={() => void permanentlyDeleteUsers([user.id])} className="inline-flex items-center gap-1.5 rounded-lg border border-red-300 px-2.5 py-2 text-xs font-bold text-red-800 hover:bg-red-50 disabled:opacity-50"><Trash2 size={14} />Futa kabisa</button>}{(!canProvision || protectedAccount) && <span className="text-xs font-semibold text-slate-400">{protectedAccount ? "Imelindwa" : "Imezimwa"}</span>}</div></td></tr>; })}</tbody></table></div>}
    </section>
  </div>;
}
