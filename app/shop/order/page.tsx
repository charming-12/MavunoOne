      ...
      <div className="mx-auto max-w-7xl px-4 py-8 md:px-8 md:py-12">
        ...
        <div className="grid gap-8 lg:grid-cols-[1.15fr_0.85fr] lg:items-start">
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            {step === 3 && <section className="rounded-[28px] border border-white/10 bg-white/[0.05] p-6 md:p-8"><div className="mb-7 flex items-start gap-4"><div className="rounded-2xl bg-amber-400/15 p-3 text-amber-300"><CreditCard size={22} /></div><div><p className="text-xs font-black uppercase tracking-widest text-amber-300">Hatua ya tatu</p><h2 className="mt-2 text-2xl font-black">Chagua malipo</h2><p className="mt-2 text-sm leading-6 text-emerald-100/65">Timu itathibitisha malipo kabla ya kuandaa delivery.</p></div></div><div className="grid gap-3">{[["cash", "Pesa taslimu", "Lipa wakati wa delivery"], ["mpesa", "M-Pesa", "Lipa kwa namba ya biashara"], ["tigopesa", "Tigo Pesa / Mixx by Yas", "Lipa kwa namba ya simu"], ["bank", "Benki", "Tuma kupitia benki yako"]].map(([value, title, description]) => <label key={value} className={`flex cursor-pointer items-center gap-4 rounded-2xl border p-4 transition ${formData.paymentMethod === value ? "border-amber-300/60 bg-amber-300/10" : "border-white/10 bg-[#06120d] hover:border-emerald-400/40"}`}><input type="radio" name="paymentMethod" value={value} checked={formData.paymentMethod === value} onChange={(e) => setField("paymentMethod", e.target.value)} className="h-4 w-4 accent-amber-400" /><span><span className="block font-black text-white">{title}</span><span className="mt-1 block text-xs text-emerald-100/60">{description}</span></span></label>)}</div>{(formData.paymentMethod === "mpesa" || formData.paymentMethod === "tigopesa") && <div className="mt-5 rounded-2xl border border-amber-300/25 bg-amber-300/10 p-4 text-sm text-amber-100">{paymentInstructions.enabled && activePayment?.number ? <><p className="font-black">Lipa TZS {total.toLocaleString()} kwenda {activePayment.number}{activePayment.name ? ` (${activePayment.name})` : ""}.</p><p className="mt-2 text-xs text-amber-100/75">Tutathibitisha oda baada ya mawasiliano ya simu.</p></> : <p>Lipa Number bado haijawekwa. Admin aweke namba kwenye Setup Wizard.</p>}</div>}</section>}
