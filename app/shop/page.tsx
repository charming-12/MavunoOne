...
export default function ShopPage() {
  ...
  const products = useMemo(() => (productsQuery.data ?? []).map((product) => ({
    ...
    baseStockKg: Number(product.currentStock ?? 0),
    unit: product.unit,
    packageSizeKg: Number(product.packageSizeKg ?? 1),
    image: imageForProduct(product.name)?.src,
    imageAlt: imageForProduct(product.name)?.alt ?? product.name,
  })), [productsQuery.data]);
