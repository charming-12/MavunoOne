          ...
          <>
            ...
            <div className="grid items-start gap-6 lg:grid-cols-[minmax(0,1fr)_360px]">
                ...
                {cart.map((item) => (
                  <article key={item.id} className="flex gap-4 rounded-2xl border border-emerald-800 bg-[#0a1e18] p-4 shadow-xl sm:p-5">
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <p className="mt-1 text-sm font-bold text-amber-300">{money(item.price)} <span className="font-medium text-slate-400">/ {item.unit || "unit"}</span></p>
              ...
              <aside className="rounded-3xl border border-emerald-800 bg-[#0a1e18] p-5 shadow-xl sm:p-6">
                ...
                <div className="mt-5 space-y-3 border-b border-emerald-800 pb-5 text-sm"><div className="flex justify-between"><span className="text-emerald-200">Bidhaa</span><strong className="text-white">{itemCount}</strong></div><div className="flex justify-between"><span className="text-emerald-200">Subtotal</span><strong className="text-white">{money(subtotal)}</strong></div>{taxSettings.enabled && <div className="flex justify-between text-emerald-200"><span>{taxSettings.label} ({taxSettings.rate}%)</span><span>{money(tax)}</span></div>}</div>
